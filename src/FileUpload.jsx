...
export default function TemplateDemo() {
    ...
    const handleSubmit = async () => {
        ...
        if (selected[0]) {
            response1 = await axios.post(`http://127.0.0.1:5000/generate-question-answers/remember/${isSelected?1:0}`, formData, {
                headers: {
        ...
        if (selected[1]) {
            response2 = await axios.post(`http://127.0.0.1:5000/generate-question-answers/understand/${isSelected ? 1 : 0}`, formData, {
                headers: {
                    "Content-Type": "multipart/form-data"
                }
            });
        ...
        if (selected[2]) {
            response3 = await axios.post(`http://127.0.0.1:5000/generate-question-answers/apply/${isSelected ? 1 : 0}`, formData, {
                headers: {
                    "Content-Type": "multipart/form-data"
                }
            });
            ...
            {!isLoading && (
                <div style={{ display: "flex", width: "100%", padding: "0.5rem 2rem" }} >
                    ...
                    <div className="vertical-line" style={{
                        borderLeft: '1px solid #B721FFF1', boxShadow: '0px 0px 5px rgba(0, 0, 0, 0.5)', height: '80vh', marginLeft: '3rem'
                    }}></div>
                    {file ?
